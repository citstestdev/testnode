...
router.get('/expertise', checkLogin, async function (req, res, next) {
  ...
  await MongoClient.connect(url, function (err, db) {
    ...
    dbo.collection('option').findOne(function (err, result) {
      if (err) { return };
      console.log(err);
      res.render('admin/home/expertise_show', { title: "Expertise", opt: result, headermenu:headermenu_dynamic, settingmenu:setting_dynamic, pagedata: expertise, expertiseitem: expertiseitem, 'msg': session.massage });
        setTimeout(function() {
    session.massage = '';
 }, 5000);

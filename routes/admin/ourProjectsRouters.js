...
router.get('/projects', checkLogin, async function (_req, res, _next) {
  ...
  await MongoClient.connect(url, function (err, db) {
    ...
    var expertiseitem = [];
    dbo.collection('ourprojectsitem').find().sort({'_id':-1}).toArray(function (_err, result2) {
   ...
   var headermenu_dynamic = [];  
    ...
    dbo.collection('option').findOne(function (err, result) {
      ...
      if (err) { return };
      console.log(err);
      res.render('admin/home/oProject_show', { title: "Our Projects", opt: result,  headermenu:headermenu_dynamic, settingmenu:setting_dynamic, pagedata: expertise, expertiseitem: expertiseitem, 'msg': session.message });
...
router.get('/projectsform', checkLogin, function (_req, res, _next) {
  ...
  MongoClient.connect(url, function (err, db) {
    ...
    var expertise = [];
    dbo.collection('ourprojects').find().toArray(function (_err, result1) {
      expertise = result1;
    });
...
router.post('/projects', checkLogin, async function (req, res, _next) {
...
router.get('/ourprojectsitemform', async function (_req, res, _next) {
...
router.post('/ourprojectsitem',upload.single('userPhoto'), async function (req, res, _next) {
...
router.get('/ourprojectsitemedit/:id', function (req, res, _next) {
  ...
  MongoClient.connect(url, function (err, db) {
    ...
    var updatearr = [];
    dbo.collection('ourprojectsitem').findOne({ "_id": ObjectID(aid) }, function (_err, result) {
    ...
    dbo.collection('option').findOne(function (err, result) {
      ...
      // res.render('header')
      // session.message = "Project item added successfully";
      res.render('admin/home/oProject_item', { title: "Our Projects Update",  headermenu:headermenu_dynamic, settingmenu:setting_dynamic, updatearr: updatearr, opt: result, 'msg': session.message });
    });
  });
...
router.post('/ourprojectsitemupdate/:id', upload.single('userPhoto'), async function (req, res, _next) {
  ...
  await MongoClient.connect(url, function (err, db) {
    ...
    collection.updateOne({ "_id": ObjectID(req.params.id) }, myobj, function (err, _result) {
      if (err) { throw err; }
...
router.get('/ourprojectsitemremove/:id', async function (req, res, _next) {
...
router.get('/ourprojectsitem-show-limit', async function(req, res, _next) {
    var fullUrl = req.protocol + '://' + req.get('host');
    await MongoClient.connect(url, function(err, db) {
      if (err) throw err;
      var dbo = db.db("conative");

    ...
    await MongoClient.connect(url, function(err, db) {
      ...
      var setting_dynamic = [];  
       ...
       dbo.collection('option').findOne(function (err, result) {
          if (err) {return};
          console.log(err);
             res.render('admin/home/startproject_show', { title:"Start Project",  headermenu:headermenu_dynamic, settingmenu:setting_dynamic, opt:result,pagedata:digitallegaciesdata, 'msg':session.message});
...
router.post('/addstartpjt', checkLogin,  upload.array('userPhoto'), async function(req, res, next) {
      ...
      await myobj.save();
    session.message = "Item inserted successfully";  

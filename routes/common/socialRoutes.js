...
router.get('/social', checkLogin, async function(req, res, _next) {
    ...
    await MongoClient.connect(url, function(err, db) {
      ...
      var setting_dynamic = [];  
       ...
       dbo.collection('option').findOne(function (err, result) {
          if (err) {return};
          console.log(err);
           res.render('admin/home/socialmedia', { title:"Social Media", headermenu:headermenu_dynamic,settingmenu:setting_dynamic,  opt:result,pagedata:socials, 'msg':session.message});
           setTimeout(function() {
            session.massage = '';
            // msg='';
         }, 5000);
...
router.post('/social', checkLogin, upload.single('userPhoto'), async function (req, res, _next) {
...
router.get('/social/:id', checkLogin, function(req, res, _next) {
    ...
    MongoClient.connect(url, function(err, db) {
      ...
      var setting_dynamic = [];  
       ...
       dbo.collection('option').findOne(function (err, result) {
          if (err) {return};
          console.log(err);
           res.render('admin/home/socialupdate', { title:"Social Edit",   headermenu:headermenu_dynamic,settingmenu:setting_dynamic,  opt:result, updatearr: updatearr, 'msg':''});
        });
    });
});
